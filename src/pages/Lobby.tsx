...
export default function Lobby() {
    ...
    if (!room)
        return <div className="lobby-page"><h1>No room found</h1></div>;

    return (
        <div className="lobby-page">
            <h1>Room: <span className="room-code-txt">{room.code}</span></h1>
            <p className="category-txt">Category: {room.category}</p>
            <p className="status-txt">{isHost ? "You are the host." : "Waiting for the host to start."}</p>

            <h3 className="player-list-title">Players</h3>
            <ul className="player-list">
                {room.players.map((p) => (
                    <li className="player-item" key={p.socketId}>{p.name}</li>
                ))}
            </ul>

            {isHost && <button className="start-button" onClick={handleStartGame}>Start Game</button>}
