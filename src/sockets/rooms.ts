...
export function createRoom(hostSocketId: string, hostName: string, category: string): Room {
    ...
    const room: Room = {
        ...
        hostSocketId,
        category,
        players: new Map([[hostSocketId, {socketId: hostSocketId, name: hostName, score: 0, multiplier: 1, answered: false, answer: null, correct: null}]]),
        status: 'waiting',
        currentRoundIndex: 0,
        currentRound: null,
...
export function joinRoom(code: string, socketId: string, name: string): Room | null {
    ...
    room.players.set(socketId, {socketId, name, score: 0, multiplier: 1, answered: false, answer: null, correct: null});
    return room;
}
...
export async function fetchRoundForCategory(category: string): Promise<RoundData | null> {
    const type: RoundType = pickRoundType();
    ...
    if (type === 'keyword') {
        ...
        return {
            type: 'keyword',
            timeLimit: getTypingTimeLimitSeconds(chosen.word),
            startTime: Date.now(),
            word: chosen.word,
            fact: chosen.fact,
        };
        ...
        return {
            type: 'multipleChoice',
            timeLimit: MULTIPLE_CHOICE_TIME_LIMIT_SECONDS,
            startTime: Date.now(),
...
export function scoreAnswer(round: RoundData, player: Player, correct: boolean): void {
    if (correct) {
        const elapsedMs = Date.now() - round.startTime;
        const roundScore = calculateScore(elapsedMs, round.timeLimit, player.multiplier);

        player.score += roundScore;
        player.multiplier = nextMultiplier(player.multiplier);
    } else {
        player.multiplier = 1;
    }
...
export function serializeRoom(room: Room) {
    return {
        ...
        players: Array.from(room.players.values()).map((p) => ({
            socketId: p.socketId, name: p.name, score: p.score, multiplier: p.multiplier,
        })),
    }
}